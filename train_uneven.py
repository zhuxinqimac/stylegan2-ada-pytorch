...
# --- Last Modified: Tue 20 Apr 2021 21:02:32 AEST
# --- Author: Xinqi Zhu
# .<.<.<.<.<.<.<.<.<.<.<.<.<.<.<.<
...
from training import training_loop_uneven
...
def setup_training_loop_kwargs(
    ...
    n_samples_per = None, # The number of steps in traversals.
    ...
    if cfg == 'auto':
        ...
        # spec.fmaps = 0.125 / 2. if res == 64 else spec.fmaps
    ...
    if w1reg_lambda is None:
        w1reg_lambda = 0.
    if uneven_reg_maxval is None:
        uneven_reg_maxval = 0.
    if uneven_reg_type is None:
        uneven_reg_type = 'linear'
    args.loss_kwargs = dnnlib.EasyDict(class_name='training.loss_uneven.UnevenLoss', r1_gamma=spec.gamma,
                                       w1reg_lambda=w1reg_lambda, uneven_reg_maxval=uneven_reg_maxval, reg_type=uneven_reg_type)
...
def subprocess_fn(rank, args, temp_dir):
    ...
    training_loop_uneven.training_loop(rank=rank, **args)
