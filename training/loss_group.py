...
# --- Last Modified: Sun 22 Aug 2021 20:46:58 AEST
# --- Author: Xinqi Zhu
# .<.<.<.<.<.<.<.<.<.<.<.<.<.<.<.<
"""
Docstring
...
class GroupGANLoss(Loss):
    ...
    def calc_basis_mul_ij(self, lie_alg_basis):
    ...
    def calc_hessian_loss(self, lie_alg_basis_mul_ij):
    ...
    def calc_commute_loss(self, lie_alg_basis_mul_ij):
    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, sync, gain):
        ...
        do_Gregalg = (phase in ['Gregalg', 'Gboth'])
        ...
        # Gregalg: Enforce commute or Hessian loss.
        if do_Gregalg:
            with torch.autograd.profiler.record_function('Compute_regalg_loss'):
                lie_alg_basis_mul_ij = self.calc_basis_mul_ij(self.G.core.lie_alg_basis)
                hessian_loss = 0.
                if self.hessian_lamb > 0:
                    hessian_loss = self.hessian_lamb * self.calc_hessian_loss(lie_alg_basis_mul_ij)
                    training_stats.report('Loss/liealg/hessian_loss', hessian_loss)
                commute_loss = 0.
                if self.commute_lamb > 0:
                    commute_loss = self.commute_lamb * self.calc_commute_loss(lie_alg_basis_mul_ij)
